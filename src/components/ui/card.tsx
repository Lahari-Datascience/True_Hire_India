...
export function Card({ className, children, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div
      className={cn(
        'bg-white rounded-xl border border-gray-200/80 shadow-sm transition-all duration-200 hover:shadow-md',
        className
      )}
      {...props}
    >
...
export function CardTitle({ className, children, ...props }: React.HTMLAttributes<HTMLHeadingElement>) {
  return <h3 className={cn('text-lg font-bold text-gray-900 tracking-tight', className)} {...props}>{children}</h3>;
}
